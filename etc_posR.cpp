 ...
 * of the etc classifier for the positive right side for a given set of parameters fp, fn, 
 * n0, n1, c0, c1, pi0. posRght calculates the starting values and calls the recursive 
...
void posRght_rec(std::map <std::vector<int>, mpz_class>& memo, mpz_class* sums, int level, int start, int stop, int& tp, int& fn, int& tn, int& fp, double& wght) {
      ...
      *sums = *sums + start - stop + 1;
      ...
      if (mxpsr >= 0) {
        ...
        for (int i = start; i >= stop; i--) {
          ...
          posRght_rec(memo, sums, level_new, start_new, stop_new, tp, fn, tn, fp, wght);
        }
        mpz_class aftr(*sums);
...
// [[Rcpp::export]]
SEXP posRght(Rcpp::NumericMatrix clst, int n0, int n1, double c0, double c1, double pi0) {
  ...
  for (int i = 0; i < ncol; i = i + 1) { 
    ...
    if (fp == 0) {
      ...
      if (mxpsr >= 0) {
        ...
        // start recursion
        posRght_rec(memo, &sums, level, start, stop, tp, fn, tn, fp, wght);
        perm[i] = sums;
      }
    }
